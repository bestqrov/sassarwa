...
export const studentLogin = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const student = await StudentService.validateCredentials(email, password);
        if (!student) {
            sendError(res, 'Invalid email or password', 'Authentication failed', 401);
            return;
        }
        ...
        sendSuccess(res, result, 'Student login successful', 200);
    } catch (error: any) {
