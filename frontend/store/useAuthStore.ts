...
type AuthState = {
    ...
    parentLogin: (email: string, password: string) => Promise<{ success: boolean; message?: string }>;
    logout: () => void;
    getMe: () => Promise<void>;
};
...
export const useAuthStore = create<AuthState>((set, get) => ({
    ...
    studentLogin: async (email, password) => {
        try {
            ...
            console.log('Student logging in with:', email);
            ...
            console.error('Student login error details:', err);
            const msg = err.response?.data?.message || err.response?.data?.error || err.message || 'Network error';
            return { success: false, message: msg };
        } finally { set({ loading: false }); }
    },
    parentLogin: async (email, password) => {
        try {
            ...
            console.log('Parent logging in with:', email);
            ...
            console.error('Parent login error details:', err);
            const msg = err.response?.data?.message || err.response?.data?.error || err.message || 'Network error';
            return { success: false, message: msg };
        } finally { set({ loading: false }); }
    },
